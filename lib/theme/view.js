...
View.prototype._precompile = function() {
  var render = this._render; // App Render Controller
  var ctx = render.context; // Use context of render controller
  var ext = pathFn.extname(this.path); // Extention
  var renderer = render.getRenderer(ext); // Get the renderer of this extention
  var data = {
    path: this.source,
  ...
  if (renderer && typeof renderer.compile === 'function') { // Check if renderer is truthy and renderer.compile is a function
    var compiled = renderer.compile(data); // run the compile
